...
RouteGuide addRouteGuide(DefParser::Net net, double x0, double y0, double x1, double y1, int metal){
    ...
    RouteGuide rg = RouteGuid(x0,y0,x1,y1,metal);
    rg.addNet(net);
    return rg;
    
}

RouteGuide searchRouteGuide(double x0, double y0, double x1, double y1, int numMetal){
...
vector<RouteGuide> parseRouteGuid(String fileName, RawDatabase r_db){
    ...
    for (std::string line; std::getline(read, line); ) {
        ...
        if (isNet){
            ...
            RouteGuide rg = searchRouteGuide(x0,y0,x1,y1,numMetal);
            if (rg != NULL) {
                rg.addNet(net);
            } else {
                
                rg = addRouteGuide(y0,x1,y1,numMetal);
                vRouteGuide.pushback(rg);
